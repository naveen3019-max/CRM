...
export async function openConversation(actor, payload) {
  ...
  const derivedScope = getConversationScopeForRoles(actor.role, otherUser.role);
  if (payload.scope && payload.scope !== derivedScope) {
    console.debug("[chat.openConversation] scope mismatch corrected", {
  ...
  let conversation = await findConversation(scope, actor.id, payload.otherUserId);
  if (!conversation) {
    const conversationId = await createConversationRecord(scope, actor.id, payload.otherUserId);
    conversation = {
      id: conversationId,
      scope,
