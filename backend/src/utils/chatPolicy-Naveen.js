...
export function validateConversationScope(scope, roleA, roleB) {
  ...
  const provided = canonicalizeRoles([roleA, roleB]).sort().join(":");
  const expected = canonicalizeRoles(expandChatRoles(allowed)).sort().join(":");
...
export function getConversationScopeForRoles(roleA, roleB) {
  ...
  for (const [scope, roles] of Object.entries(allowedScopes)) {
    const expected = canonicalizeRoles(expandChatRoles(roles)).sort().join(":");
    if (provided === expected) {
      return scope;
    }
  }
