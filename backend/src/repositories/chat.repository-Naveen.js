...
async function executeQueryWithFallback(query, fallbackQuery, params) {
  try {
    ...
    if (err.code === 'ER_BAD_FIELD_ERROR' && (err.message.includes('original_message') || err.message.includes('latitude') || err.message.includes('map_url') || err.message.includes('address'))) {
