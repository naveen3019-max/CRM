...
export const getConversationById = asyncHandler(async (req, res) => {
  ...
  const data = await getConversation(req.user, id);
  res.json({ success: true, data });
});
...
export const sendMessage = asyncHandler(async (req, res) => {
  const uploadedImageUrl = resolveUploadedFileUrl(req.file);
  const location = req.body?.location || {};
  const data = await sendConversationMessage(req.user, {
    ...req.body,
    receiverId: Number(req.body.receiverId),
    imageUrl: uploadedImageUrl,
    latitude: location.latitude ?? req.body.latitude ?? null,
    longitude: location.longitude ?? req.body.longitude ?? null,
    address: location.address ?? req.body.address ?? null,
    mapUrl: location.mapUrl ?? location.map_url ?? req.body.mapUrl ?? req.body.map_url ?? null
  });
  res.status(201).json({ success: true, data });
});
