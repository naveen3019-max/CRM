...
function formatDuration(ms) {
  const totalSeconds = Math.max(0, Math.round(ms / 1000 || 0));
...
export function RealtimeCRMChat({
  ...
  const [microphonePermissionBlocked, setMicrophonePermissionBlocked] = useState(false);
  ...
  const openNativeSettings = () => {
    try {
      if (typeof window !== "undefined" && window.ReactNativeWebView?.postMessage) {
        window.ReactNativeWebView.postMessage(JSON.stringify({ type: "open-settings" }));
      }
    } catch (_error) {
      // No native bridge in the desktop web app.
    }
  };
  ...
  const setAttachmentFromFile = (file, previewUrl, revokePreview) => {
    setSelectedAttachment((previous) => {
      ...
      const kind = String(file.type || "").startsWith("audio/") ? "audio" : "image";
      ...
      // If audio, attempt to read duration metadata for a nicer UI
      if (kind === "audio") {
        try {
          const audio = new Audio(previewUrl);
          audio.addEventListener("loadedmetadata", () => {
            setSelectedAttachment((prev) => {
              // if the attachment changed in the meantime, preserve it
              if (!prev || prev.previewUrl !== previewUrl) return prev;
              return { ...prev, durationSeconds: Math.round(audio.duration || 0) };
            });
            audio.remove();
          });
        } catch (_e) {
  ...
  const handleStartRecording = async () => {
    ...
    try {
      ...
      setMicrophonePermissionBlocked(false);
      setAttachmentError("");
    } catch (error) {
      if (recordingStreamRef.current) {
        recordingStreamRef.current.getTracks().forEach((track) => track.stop());
      ...
      const errorText = String(error?.name || error?.message || "").toLowerCase();
      const permissionDenied = errorText.includes("notallowed") || errorText.includes("permission") || errorText.includes("denied");

      setMicrophonePermissionBlocked(permissionDenied);
      setAttachmentError(permissionDenied ? t("chat.microphonePermissionRequired") : t("chat.unableToCaptureVoiceMessage"));
    }
  };
  ...
  const handleCancelRecording = () => {
    ...
    try {
      if (recorder && recorder.state !== "inactive") {
        // Prevent onstop from creating an attachment
        recorder.onstop = null;
        recorder.stop();
      }
    } catch (e) {
  ...
  const sendSelectedAttachment = async () => {
    if (!selectedAttachment || disabledComposer || isSending) return;
    await Promise.resolve(onSend("", selectedAttachment.previewUrl || null, selectedAttachment.file || null, selectedAttachment.kind || "image"));
    setInputValue("");
  ...
  const handleSendDraftLocation = async () => {
    ...
    const locationMessage = buildLocationMessageBody(locationDraft);

    await Promise.resolve(onSend(locationMessage, null, null, "location"));
    onTypingChange?.(false);
    setInputValue("");
  ...
  const handleSendManualLocation = async () => {
    ...
    await Promise.resolve(onSend(buildLocationMessageBody({ label: manualLabel }), null, null, "location"));
    onTypingChange?.(false);
    setInputValue("");
    setLocationPickerOpen(false);
    setQuickActionsOpen(false);
        ...
        <div className="border-t border-gray-200 bg-white px-3 py-3 sm:px-4">
          ...
          {selectedAttachment ? (
            <div className="mb-2 flex items-center gap-3 rounded-xl border border-gray-200 bg-white px-3 py-2">
              <div className="h-10 w-10 shrink-0 rounded-lg bg-slate-100 p-2 text-slate-600 flex items-center justify-center">
                {selectedAttachment.kind === "audio" ? <Mic className="h-5 w-5" /> : <img src={selectedAttachment.previewUrl} alt="Selected file" className="h-10 w-10 rounded-md object-cover" />}
              </div>

              <div className="min-w-0 flex-1">
                <div className="flex items-center justify-between gap-3">
                  <p className="truncate text-sm font-semibold text-slate-900">{selectedAttachment.name}</p>
                  {selectedAttachment.durationSeconds !== null ? (
                    <span className="text-xs text-slate-500">{formatDuration((selectedAttachment.durationSeconds || 0) * 1000)}</span>
                  ) : null}
                </div>

                {selectedAttachment.kind === "audio" ? (
                  <audio controls preload="metadata" src={selectedAttachment.previewUrl} className="mt-2 w-full" />
                ) : null}
              </div>
              ...
              <div className="flex flex-col items-center gap-2">
                <button
                  type="button"
                  onClick={sendSelectedAttachment}
                  disabled={disabledComposer || isSending}
                  className="inline-flex h-9 w-9 items-center justify-center rounded-full bg-blue-600 text-white hover:bg-blue-700 disabled:opacity-60"
                  title="Send"
                >
          ...
          {microphonePermissionBlocked ? (
            <div className="mb-2 rounded-xl border border-rose-200 bg-rose-50 px-3 py-3">
              <p className="text-xs font-semibold text-rose-700">Microphone permission required</p>
              <p className="mt-1 text-xs text-rose-700/90">Allow microphone access to record and send voice messages.</p>
              <div className="mt-3 flex flex-wrap gap-2">
                ...
                <button
                  ...
                  className="rounded-full border border-rose-300 bg-white px-3 py-1.5 text-xs font-semibold text-rose-700 transition hover:border-rose-400 hover:text-rose-800"
                >
                  Open Settings
                </button>
              </div>
          ...
          {isRecording ? (
            <div className="mb-2 flex items-center justify-between gap-3 rounded-xl border border-rose-200 bg-rose-50 px-3 py-2">
              <div className="flex items-center gap-3">
                <span className="inline-block h-3 w-3 rounded-full bg-rose-600 animate-pulse" />
                <div>
                  <p className="text-sm font-semibold text-rose-700">{t("chat.recordingVoiceMessage")}</p>
                  <p className="text-xs text-rose-700/90">{formatDuration(recordingElapsed)}</p>
                </div>
              </div>
              ...
              <div className="flex items-center gap-2">
                ...
                <button
                  ...
                  className="inline-flex items-center gap-2 rounded-full border border-rose-200 bg-white px-3 py-1 text-xs font-semibold text-rose-700 hover:bg-rose-50"
                >
                  Cancel
                </button>
              </div>
