...
function MessageCard({ message, currentUserId, onPinMessage, onUnpinMessage, isHighlighted = false, pinnedOverride = null }) {
  ...
  const parsedLocation =
    messageType === "location"
      ? parseLocationPayload(displayBody, message.metadataJson || message.metadata || message || null)
      : null;
...
export function RealtimeCRMChat({
  ...
  const toggleLocationPicker = () => {
    ...
    setLocationPickerOpen((previous) => !previous);
  };
    ...
    <div className="grid h-full min-h-0 grid-cols-1 xl:grid-cols-[360px_1fr]">
      ...
      >
        ...
        <div className="border-t border-gray-200 bg-white px-3 py-3 sm:px-4">
          ...
          {locationDraft ? (
            <div className="mb-2 rounded-xl border border-emerald-200 bg-emerald-50 px-3 py-3">
              <div className="flex items-start justify-between gap-3">
                <div className="min-w-0">
                  <p className="text-xs font-semibold text-emerald-700">Selected location</p>
                  <p className="mt-1 text-xs text-emerald-800">{locationDraft.formattedAddress || locationDraft.label}</p>
                  {locationDraft.latitude != null && locationDraft.longitude != null ? (
                    <p className="mt-1 text-[11px] text-emerald-700">
                      {Number(locationDraft.latitude).toFixed(5)}, {Number(locationDraft.longitude).toFixed(5)}
                    </p>
                  ) : null}
                </div>
                <button
              ...
              <div className="mt-3 flex flex-wrap gap-2">
                ...
                {locationDraft.mapUrl ? (
                  <a
                    href={locationDraft.mapUrl}
                    target="_blank"
                    rel="noreferrer"
          ...
          <form onSubmit={handleSubmit} className="space-y-2 sm:space-y-0">
            ...
            <div className="flex items-center gap-2">
              <input
                ...
                disabled={disabledComposer}
                placeholder="Type a message, or share a location"
              ...
              <button
                type="submit"
                disabled={disabledComposer || isSending || isRecording || (!inputValue.trim() && !selectedAttachment)}
          ...
          <LocationPicker
            ...
            title="Share location"
            subtitle="Use GPS, search for a place, and confirm the exact pin before sending."
            ...
            confirmLabel={locationDraft ? "Update location" : "Use this location"}
          />
        </div>
      </section>
